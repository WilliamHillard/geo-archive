...
def transform_internet_users():
    input_file = RAW_DATA_DIR / "internet_users_raw.json"
    ...
    df["internet_users"] = df["internet_users"].astype(float)

    output = CLEANED_DATA_DIR / "internet_users.csv"
    df.to_csv(output, index=False)

    print(f"\nCleaned internet users data saved!")
